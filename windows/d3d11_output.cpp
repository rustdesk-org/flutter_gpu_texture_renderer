...
D3D11Output::D3D11Output(flutter::TextureRegistrar *texture_registrar,IDXGIAdapter *adapter) :
  ...
  MS_THROW(dev_->CreateTexture2D(&desc, nullptr, tex_.GetAddressOf()));
...
void D3D11Output::runDup() {
  ...
  DDAImpl dda(dev_.Get(), ctx_.Get());
  dda.Init();

  while (!stopThread_) {
    ComPtr<ID3D11Texture2D> texture;
    if(SUCCEEDED(dda.GetCapturedFrame(texture.GetAddressOf(), 100))) {
          ctx_->CopyResource(tex_.Get(), texture.Get());
          Present();
    }
  }
}
